...
def read_mean_and_covariances(filename):
  ...
  mu = np.array(data.feature['mu'].float_list.value)
  emb_len = np.array(
      data.feature['embedding_length'].int64_list.value)[0]
  sigma = (np.array(
      data.feature['sigma'].float_list.value)).reshape((emb_len,emb_len))
  return mu, sigma
