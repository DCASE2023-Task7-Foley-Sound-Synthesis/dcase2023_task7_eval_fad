...
class FADWrapper:
        ...
        for name_of_sound in self.name_of_sound_list:
            ...
            for set_type in self.audio_dir:
                if not self.exist_mean_var_dict[set_type]:
                    ...
                    files_path_list:list = [f'{self.audio_dir[set_type]}/{name_of_sound}/{file_name}' for file_name in os.listdir(f'{self.audio_dir[set_type]}/{name_of_sound}') if os.path.splitext(file_name)[-1] == '.wav']
                    self.sanity_check(files_path_list)
                    self.write_meta_data(save_dir=embeddig_output_dir,save_file_name=f'{set_type}_files.cvs',file_path_list=files_path_list)
    ...
    def sanity_check(self,file_path_list:List[str]) -> None:
        assert len(file_path_list) == self.number_of_audio_in_each_dir, f"[Error]The number of audio is {len(audio_dir)}. there should be exactly {self.number_of_audio_in_each_dir} wav files in each subfolder."
        for file_path in file_path_list:
            audio_data, sr = librosa.load(file_path, sr=None, mono=False)
            assert sr == self.sample_rate, f"sample rate should be {self.sample_rate}, but found {sr} at {file_path}."
            assert audio_data.ndim == 1, f"audio should be mono, but this file seems not: {file_path}"
            assert len(audio_data) == self.sample_rate * self.audio_length_sec, f"length is expected to be 88200, but found {len(audio_data)}."
    
    def write_meta_data(self,save_dir:str,save_file_name:str,file_path_list:List[str]) -> None:
